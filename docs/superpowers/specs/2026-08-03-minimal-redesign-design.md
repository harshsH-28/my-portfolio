...
- **Drop reading page (user request, 2026-08-18)** — drops that carry a body (blog-style notes) get their own page, styled after the reference at `assets/drop-page-reference.png` (a Lee Robinson-style post page): narrow centered reading column on plain `paper`, serif post title, small byline row (date · author) in mono/faint, serif or calm body text with generous line-height, quiet underlined links, serif section headings. Media allowed but minimal: images, GIFs, or short videos inline in the flow, full column width, nothing fancy — "simple things in a simple manner, clean and sleek." No sidebars, no cards, no share widgets.
- **Out of scope for phase 2:** editing, drafts — add only when actually needed. (Markdown/media rendering moves IN scope only as far as the reading page above needs it.)
